...
//   CONTEXTDEV_API_KEY=… npm -w @contextdev/core run cartographer -- "<category>" [--max N] [--json <path>] [--export-webdog <path>]
...
import { toWebdogWatchlist } from "../landscape/webdog.js";

const argv = process.argv.slice(2);
const VALUE_FLAGS = new Set(["--max", "--json", "--seed", "--export-webdog"]);
...
if (!category) { console.error('Usage: npm -w @contextdev/core run cartographer -- "<category>" [--max N] [--json <path>] [--export-webdog <path>] [--seed <domain> ...]'); process.exit(1); }
...
const webdogPath = flag("--export-webdog");
...
if (webdogPath) { writeFileSync(webdogPath, JSON.stringify(toWebdogWatchlist(landscape), null, 2) + "\n"); console.error(`\nwebdog watch-list -> ${webdogPath}`); }
